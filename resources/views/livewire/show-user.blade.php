<div class="flex flex-col lg:flex-row w-full h-full space-y-4 lg:space-x-4 lg:space-y-0 p-2">
    ...
    <div class="flex flex-col sm:flex-row lg:flex-col w-full lg:w-1/3 space-y-4 sm:space-x-4 lg:space-y-4 lg:space-x-0">
        {{-- Name Card --}}
        <div
            class="flex flex-col sm:w-1/2 lg:w-full p-4 space-y-4 overflow-hidden bg-white border border-gray-100 rounded-lg shadow-md">

            <div class="flex items-center justify-center mx-auto overflow-hidden rounded-full h-fit w-fit relative">
                <img style="object-fit: cover;height: 100px; width: 100px" src="{{ $user->profile_photo_url }}"
                    alt="{{ $user->name }}">
            </div>
            ...
            <div class="flex flex-col justify-between flex-auto space-y-2">
                <div class="flex flex-row justify-between ">
                    ...
                    <div class="text-xs italic text-gray-400">Bergabung pada
                        {{ $user->created_at->TranslatedFormat('d F Y') }}</div>
                </div>
                <div class="flex flex-col">
                    <span
                        class="px-2 text-sm text-white {{ $user->role == 0 ? 'bg-gray-400' : 'bg-green-500' }} rounded-lg shadow-md w-fit">{{ $user->role == 0 ? 'Pegawai' : 'Admin' }}</span>
                    <span class="text-2xl font-semibold text-gray-800">{{ $user->name }}</span>
                    <span class="text-sm text-gray-500">{{ $user->email }}</span>
                </div>
                <div class="flex flex-row space-x-1">
                    <button
                        class="flex items-center justify-center text-lg text-white bg-green-500 rounded-lg shadow-md w-7 h-7"><i
                            class="fab fa-whatsapp"></i></button>
                    <button
                        class="flex items-center justify-center text-white bg-blue-500 rounded-lg shadow-md w-7 h-7"><i
        ...
        <div class="flex flex-col flex-auto justify-between space-y-4">
            ...
            <div class="flex flex-row flex-auto w-full bg-white border border-gray-100 rounded-lg shadow-md">
                <div class="flex flex-col w-1/2 p-4 justify-items-stretch">
                    ...
                    @endphp
                    <span
                        class="text-sm {{ $prodTotal == 0 ? 'text-gray-500' : ($prodTotal > 0 ? 'text-green-500' : 'text-red-500') }}">
                        {{ $prodTotal == 0 ? '--' : ($prodTotal > 0 ? '▲' : '▼') }}
                        {{ $prodTotal == 0 ? 0 : ($prodTotal > 0 ? '+' . $prodTotal : $prodTotal) }}%
                    </span>
                    <span class="flex items-end flex-auto text-xl font-semibold text-gray-600">Rp.
                        {{ number_format($prodWeek->sum('quantity') * 500, '0', '', '.') }}</span>
                </div>
            ...
            <div class="flex flex-row flex-auto w-full bg-white border border-gray-100 rounded-lg shadow-md">
                <div class="flex flex-col w-1/2 p-4">
                    ...
                    <span class="text-sm text-gray-400">7 produksi terakhir</span>
                    <span
                        class="text-sm {{ $prodTotal >= 0 ? 'text-green-500' : 'text-red-500' }}">{{ $prodTotal >= 0 ? '▲' : '▼' }}
                        {{ $prodTotal >= 0 ? '+' . $prodTotal : $prodTotal }}%</span>
                    <span
                        class="flex items-end flex-auto text-xl font-semibold text-gray-600">{{ number_format($prodWeek->sum('quantity'), '0', '', '.') }}</span>
                </div>
            ...
            <div class="flex flex-row justify-between text-xs text-white">
                ...
                <div class="relative w-1/4 text-gray-600">
                    <input wire:model="search"
                        class="h-10 px-5 pr-16 text-sm bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-0"
                        type="search" name="search" placeholder="Cari produksi...">
                </div>
                <div class="flex flex-row items-center justify-center space-x-1">
                    @if ($role == 1)
                        <button
                            onclick="Livewire.emit('openModal', 'create-production', {{ json_encode(['user_id' => $user->id]) }})"
                            class="flex items-center justify-center bg-blue-500 rounded-lg hover:bg-blue-600 h-7 w-7">
                            <i class="fas fa-plus"></i>
                        </button>

                        <a href="export/{{ $user->id }}" target="_blank">
                            <button
                                class="flex items-center justify-center bg-red-500 rounded-lg hover:bg-red-600 h-7 w-7">
            ...
            <table class="w-full divide-y divide-gray-200 rounded-lg table-auto h-fit">
                <thead class="sticky top-0 bg-gray-50">
                    <tr>
                        ...
                        <td scope="col"
                            class="hidden px-6 py-3 text-xs font-medium text-center text-gray-500 uppercase sm:table-cell">
                            Tanggal</td>
                        @if ($role == 1)
                            <td scope="col"
                                class="px-6 py-3 text-xs font-medium text-center text-gray-500 uppercase">Aksi</td>
                        @endif
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($productions as $prod)
                        <tr>
                            ...
                            {{-- Qty --}}
                            <td scope="col"
                                class="px-6 {{ $updateState == 0 ? 'py-4' : 'py0' }} text-sm text-right text-gray-900">
                                @if ($updateState == $prod->id)
                                    <form action="update" wire:submit.prevent="update({{ $prod->id }})" x-data
                                        x-init="$refs.answer.focus()">
                                        <input x-ref="answer" type="number" wire:model="prodModel"
                                            class="rounded-lg focus:outline-none focus:ring-0 text-right h-fit"
                                            name="quantity" autofocus required>
                                    </form>
                                @else
                                    {{ $prod->quantity }}
                                @endif
                            ...
                            <td scope="col" class="flex-col px-6 py-4 text-sm text-gray-900 hidden sm:table-cell">
                                ...
                                <span
                                    class="text-xs text-gray-500 hidden md:block">{{ $prod->created_at->diffForHumans() }}</span>
                            </td>
                            {{-- Aksi --}}
                            @if ($role == 1)
                                <td scope="col" class="px-6 py-4 space-x-1 text-white">
                                    <div class="flex flex-row items-center justify-center space-x-1">
                                        ...
                                        @else
                                            <button wire:click="showUpdateForm({{ $prod->id }})"
                                                class="flex items-center justify-center text-xs transition duration-200 transform bg-blue-500 rounded-lg hover:bg-blue-600 h-7 w-7">
                                                <i class="fas fa-pen"></i>
                                            </button>

                                            <button wire:click="deleteModal({{ $prod->id }})"
